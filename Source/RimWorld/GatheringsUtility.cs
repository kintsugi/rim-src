...
public static class GatheringsUtility
{
	...
	public static bool TryFindRandomCellInGatheringArea(Pawn pawn, Predicate<IntVec3> cellValidator, out IntVec3 result)
	{
		...
		return CellFinder.TryFindRandomReachableCellNear(cell, pawn.Map, 8f, TraverseParms.For(TraverseMode.NoPassClosedDoors), (IntVec3 x) => validator(x), null, out result, 10);
	}
	...
	public static bool TryFindRandomCellInGatheringAreaWithRadius(Pawn pawn, float radius, Predicate<IntVec3> cellValidator, out IntVec3 result)
	{
		...
		return CellFinder.TryFindRandomReachableCellNear(partySpot, pawn.Map, radius, TraverseParms.For(TraverseMode.NoPassClosedDoors), (IntVec3 x) => validator(x), null, out result, 10);
	}

	public static bool TryFindRandomCellAroundTarget(Pawn pawn, Thing target, out IntVec3 result)
	{
		if (!target.OccupiedRect().ExpandedBy(1).EdgeCells.Where((IntVec3 c) => c.InBounds(pawn.Map) && c.Standable(pawn.Map) && !c.IsForbidden(pawn) && pawn.CanReserveAndReach(c, PathEndMode.OnCell, Danger.None)).TryRandomElement(out result))
		{

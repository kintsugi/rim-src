...
public class MultiPawnGotoController
{
	...
	public void ProcessInputEvents()
	{
		...
		if (!Active || !pawns.Any() || !intVec.InBounds(pawns[0].Map))
		{
			return;
		}
	...
	public void Draw()
	{
		...
		float addedAltitude = num + 3f / 74f;
		float addedAltitude2 = num - 3f / 74f;
		for (int i = 0; i < pawns.Count; i++)
		{
			Pawn pawn = pawns[i];
			IntVec3 c = dests[i];
			if (c.IsValid && pawn.Spawned && !c.Fogged(pawn.Map))
			{
				...
				pawn.Drawer.renderer.RenderPawnAt(drawLoc, Rot4.South, neverAimWeapon: true);

...
public class TargetingParameters
{
	...
	public bool CanTarget(TargetInfo targ, ITargetingSource source = null)
	{
		...
		if (canTargetCorpses && targ.Thing is Corpse corpse)
		{
		...
		if (canTargetPawns && targ.Thing.def.category == ThingCategory.Pawn)
		{
			...
			if (onlyTargetSameIdeo)
			{
				...
				else if (source is Verb verb && verb.CasterPawn != null)
				{
	...
	public static TargetingParameters ForQuestPawnsWhoWillJoinColony(Pawn p)
	{
		return new TargetingParameters
		{
			...
			canTargetBuildings = false,
			mapObjectTargetsMustBeAutoAttackable = false,
			validator = (TargetInfo x) => x.Thing is Pawn pawn && !pawn.Dead && pawn.mindState.WillJoinColonyIfRescued
		};
	}
	...
	public static TargetingParameters ForShuttle(Pawn hauler)
	{
		return new TargetingParameters
		{
			...
			validator = delegate(TargetInfo targ)
			{
				...
				if (!(targ.Thing is Pawn pawn) || pawn.Dead || pawn == hauler)
				{
					return false;
				}
				if (pawn.Downed)
	...
	public static TargetingParameters ForTend(Pawn doctor)
	{
		return new TargetingParameters
		{
			...
			validator = delegate(TargetInfo targ)
			{
				...
				if (!(targ.Thing is Pawn pawn) || pawn.IsWildMan())
				{
					return false;
				}
				if (pawn.Downed)

...
public class ColonistBarDrawLocsFinder
{
	...
	{
		CalculateDrawLocs(outDrawLocs, out scale, CalculateGroupsCount());
	}
	...
	{
		CalculateColonistsInGroup(CalculateGroupsCount());
	}
	...
	{
		return FindBestScale(out onlyOneRow, out maxPerGlobalRow, CalculateGroupsCount());
	}
	...
	{
		return TryDistributeHorizontalSlotsBetweenGroups(maxPerGlobalRow, CalculateGroupsCount());
	}
	...
	{
		CalculateDrawLocs(outDrawLocs, scale, onlyOneRow, maxPerGlobalRow, CalculateGroupsCount());
	}

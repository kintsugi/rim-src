...
public class TilePicker
{
	...
	public void TileSelectorOnGUI()
	{
		...
		if (Widgets.ButtonText(new Rect(rect.x + 8f, rect.y + 8f, buttonSize.x, buttonSize.y), "SelectRandomSite".Translate()))
		{
			SoundDefOf.Click.PlayOneShotOnCamera();
			Find.WorldInterface.SelectedTile = TileFinder.RandomStartingTile();
			Find.WorldCameraDriver.JumpTo(Find.WorldGrid.GetTileCenter(Find.WorldInterface.SelectedTile));
		}
		if (Widgets.ButtonText(new Rect(rect.x + 16f + buttonSize.x, rect.y + 8f, buttonSize.x, buttonSize.y), "Next".Translate()))
		{
			SoundDefOf.Click.PlayOneShotOnCamera();

...
public class CompAbilityEffect_GiveMentalState : CompAbilityEffect
{
	...
	public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
	{
		...
		{
			TryGiveMentalStateWithDuration(pawn.RaceProps.IsMechanoid ? (Props.stateDefForMechs ?? Props.stateDef) : Props.stateDef, pawn, parent.def, Props.durationMultiplier, parent.pawn);
	...
	{
		if (p.mindState.mentalStateHandler.TryStartMentalState(def, null, forceWake: true, causedByMood: false, null, transitionSilently: false, causedByDamage: false, ability.IsPsycast))
		{

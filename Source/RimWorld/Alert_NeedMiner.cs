...
public class Alert_NeedMiner : Alert
{
	...
	public override AlertReport GetReport()
	{
		...
		for (int i = 0; i < maps.Count; i++)
		{
			...
			List<Designation> list = map.designationManager.designationsByDef[DesignationDefOf.Mine];
			if (list.NullOrEmpty())
			{
				continue;
			}
			...
			if (!flag)
			{
				return AlertReport.CulpritIs(new GlobalTargetInfo(list[0].target.Cell, map));
			}
		}
		return false;
	}

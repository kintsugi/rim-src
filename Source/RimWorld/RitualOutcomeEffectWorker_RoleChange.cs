...
public class RitualOutcomeEffectWorker_RoleChange : RitualOutcomeEffectWorker
{
	...
	public override void Apply(float progress, Dictionary<Pawn, int> totalPresence, LordJob_Ritual jobRitual)
	{
		...
		bool num2 = num > 0.5f;
		OutcomeChance outcomeChance = (num2 ? def.outcomeChances[1] : def.outcomeChances[0]);
		string text = outcomeChance.description.Formatted(jobRitual.Ritual.Label, jobRitual.Ritual.ideo.Named("IDEO")).CapitalizeFirst();
		...
		Find.LetterStack.ReceiveLetter("OutcomeLetterLabel".Translate(outcomeChance.label.Named("OUTCOMELABEL"), jobRitual.Ritual.Label.Named("RITUALLABEL")), text, outcomeChance.Positive ? LetterDefOf.RitualOutcomePositive : LetterDefOf.RitualOutcomeNegative, jobRitual.selectedTarget);
		if (num2)
		{
	...
	{
		RitualOutcomeComp ritualOutcomeComp = def.comps.First();
		if (!(ritualOutcomeComp.GetExpectedOutcomeDesc(ritual, ritualTarget, obligation, assignments, DataForComp(ritualOutcomeComp)).quality > 0.5f))
		{
	...
	public override IEnumerable<string> BlockingIssues(Precept_Ritual ritual, TargetInfo target, RitualRoleAssignments assignments)
	{
		...
		if (ritualOutcomeComp.GetExpectedOutcomeDesc(ritual, target, null, assignments, ritualOutcomeComp_DataRoleChangeParticipants).quality < 0.5f)
		{

...
public static class StorageGroupUtility
{
	...
	public static IEnumerable<Gizmo> StorageGroupMemberGizmos(IStorageGroupMember member)
	{
		...
		{
			if (!(selectedObject is IStorageGroupMember storageGroupMember) || !(storageGroupMember.StorageGroupTag == member.StorageGroupTag) || (storageGroupMember is Building_Storage building_Storage && !building_Storage.StorageTabVisible) || storageGroupMember.Map != member.Map)
			{
				continue;
			}

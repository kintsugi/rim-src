...
public static class FactionDialogMaker
{
	...
	private static DiaOption RequestRoyalHeirChangeOption(Map map, Faction faction, Pawn factionRepresentative, Pawn negotiator)
	{
		...
		bool num = Find.QuestManager.QuestsListForReading.Any((Quest q) => q.root == QuestScriptDefOf.ChangeRoyalHeir && q.State == QuestState.Ongoing && q.PartsListForReading.Any((QuestPart p) => p is QuestPart_ChangeHeir questPart_ChangeHeir && !questPart_ChangeHeir.done && questPart_ChangeHeir.holder == negotiator));

...
public class JoyGiver_SocialRelax : JoyGiver
{
	...
	private static bool TryFindChairBesideTable(Thing table, Pawn sitter, out Thing chair)
	{
		for (int i = 0; i < 30; i++)
		{
			...
			if (edifice != null && edifice.def.building.isSittable && sitter.CanReserve(edifice) && (compAssignableToPawn_Throne == null || compAssignableToPawn_Throne.AssignedPawns.Contains(sitter)))
			{
				chair = edifice;
				return true;
			}
	...
	private static bool TryFindChairNear(IntVec3 center, Pawn sitter, out Thing chair)
	{
		for (int i = 0; i < RadialPatternMiddleOutward.Count; i++)
		{
			...
			CompAssignableToPawn_Throne compAssignableToPawn_Throne = edifice.TryGetComp<CompAssignableToPawn_Throne>();
			if (edifice != null && edifice.def.building.isSittable && sitter.CanReserve(edifice) && !edifice.IsForbidden(sitter) && GenSight.LineOfSight(center, edifice.Position, sitter.Map, skipFirstCell: true) && (compAssignableToPawn_Throne == null || compAssignableToPawn_Throne.AssignedPawns.Contains(sitter)))
			{
				chair = edifice;
				return true;
			}

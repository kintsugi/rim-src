...
public abstract class Thought : IExposable
{
	...
	public virtual string Description
	{
		get
		{
			...
			description = ((def.Worker != null) ? def.Worker.PostProcessDescription(pawn, description) : ((this is Thought_Memory thought_Memory && thought_Memory.otherPawn != null) ? description.Formatted(pawn.Named("PAWN"), thought_Memory.otherPawn.Named("OTHERPAWN")).Resolve() : ((!(this is ISocialThought socialThought) || socialThought.OtherPawn() == null) ? description.Formatted(pawn.Named("PAWN")).Resolve() : description.Formatted(pawn.Named("PAWN"), socialThought.OtherPawn().Named("OTHERPAWN")).Resolve())));

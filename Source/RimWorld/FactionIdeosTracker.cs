...
public class FactionIdeosTracker : IExposable
{
	...
	[Obsolete]
	public int LastAnimalSlaughterTick => 0;
	...
	public void ChooseOrGenerateIdeo(IdeoGenerationParms parms)
	{
		...
		if ((Rand.Chance(0.2f) || Find.IdeoManager.IdeosListForReading.Count >= 10 || faction.def.hidden) && Find.IdeoManager.IdeosListForReading.Where((Ideo x) => IdeoUtility.CanUseIdeo(faction.def, x, parms.disallowedPrecepts)).TryRandomElement(out var result))
		{
			primaryIdeo = result;
			return;
		}
		Ideo ideo2 = IdeoGenerator.GenerateIdeo(parms);

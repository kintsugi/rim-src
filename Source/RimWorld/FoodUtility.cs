...
public static class FoodUtility
{
	...
	public static bool WillEat_NewTemp(this Pawn p, Thing food, Pawn getter = null, bool careIfNotAcceptableForTitle = true, bool allowVenerated = false)
	{
		if (!p.WillEat_NewTemp(food.def, getter, careIfNotAcceptableForTitle, allowVenerated))
		{
			return false;
		}
		...
		if (p.foodRestriction != null && !p.DevelopmentalStage.Baby())
		{
	...
	{
		return p.WillEat_NewTemp(food, getter, careIfNotAcceptableForTitle);
	}

	public static bool WillEat_NewTemp(this Pawn p, ThingDef food, Pawn getter = null, bool careIfNotAcceptableForTitle = true, bool allowVenerated = false)
	{
	...
	public static bool TryFindBestFoodSourceFor_NewTemp(Pawn getter, Pawn eater, bool desperate, out Thing foodSource, out ThingDef foodDef, bool canRefillDispenser = true, bool canUseInventory = true, bool canUsePackAnimalInventory = false, bool allowForbidden = false, bool allowCorpse = true, bool allowSociallyImproper = false, bool allowHarvest = false, bool forceScanWholeMap = false, bool ignoreReservations = false, bool calculateWantedStackCount = false, bool allowVenerated = false, FoodPreferability minPrefOverride = FoodPreferability.Undefined)
	{
		...
		{
			if (allowVenerated && TryFindBestFoodSourceFor_NewTemp(getter, eater, desperate, out foodSource, out foodDef, canRefillDispenser, canUseInventory, canUsePackAnimalInventory, allowForbidden, allowCorpse, allowSociallyImproper, allowHarvest, forceScanWholeMap, ignoreReservations, calculateWantedStackCount, allowVenerated: false, minPrefOverride))
			{
				return true;
			}
			...
			if (canUseInventory)
			{
				...
				{
					thing = BestFoodInInventory_NewTemp(getter, eater, (minPrefOverride == FoodPreferability.Undefined) ? FoodPreferability.MealAwful : minPrefOverride, FoodPreferability.MealLavish, 0f, allowDrug: false, allowVenerated);
				}
				if (thing != null)
				{
			...
			ThingDef foodDef2;
			Thing thing2 = BestFoodSourceOnMap_NewTemp(getter2, eater2, desperate, out foodDef2, FoodPreferability.MealLavish, allowPlant, allowDrug, allowCorpse, allowDispenserFull: true, canRefillDispenser, allowForbidden2, allowSociallyImproper, allowHarvest, forceScanWholeMap, ignoreReservations, calculateWantedStackCount, minPrefOverride, null, allowVenerated);
			...
			{
				thing = BestFoodInInventory_NewTemp(getter, eater, FoodPreferability.DesperateOnly, FoodPreferability.MealLavish, 0f, allowDrug);
				if (thing == null)
				{
			...
			Thing FirstFoodInClosestPackAnimalInventory(FoodPreferability foodPref)
			{
				...
				if (canUseInventory && canUsePackAnimalInventory && canManipulateTools && eater.IsColonist && getter.IsColonist && getter.Map != null)
				{
					...
					{
						thing3 = BestFoodInInventory_NewTemp(spawnedColonyAnimal, eater, foodPref, FoodPreferability.MealLavish, 0f, allowDrug: false, allowVenerated);
	...
	public static ThingDef GetFinalIngestibleDef(Thing foodSource, bool harvest = false)
	{
		...
		if (harvest && foodSource is Plant plant && plant.HarvestableNow && plant.def.plant.harvestedThingDef.IsIngestible)
		{
	...
	{
		return BestFoodInInventory_NewTemp(holder, eater, minFoodPref, maxFoodPref, minStackNutrition, allowDrug);
	}
	...
	{
		return BestFoodSourceOnMap_NewTemp(getter, eater, desperate, out foodDef, maxPref, allowPlant, allowDrug, allowCorpse, allowDispenserFull, allowDispenserEmpty, allowForbidden, allowSociallyImproper, allowHarvest, forceScanWholeMap, ignoreReservations, calculateWantedStackCount, minPrefOverride, minNutrition);
	}

	public static Thing BestFoodSourceOnMap_NewTemp(Pawn getter, Pawn eater, bool desperate, out ThingDef foodDef, FoodPreferability maxPref = FoodPreferability.MealLavish, bool allowPlant = true, bool allowDrug = true, bool allowCorpse = true, bool allowDispenserFull = true, bool allowDispenserEmpty = true, bool allowForbidden = false, bool allowSociallyImproper = false, bool allowHarvest = false, bool forceScanWholeMap = false, bool ignoreReservations = false, bool calculateWantedStackCount = false, FoodPreferability minPrefOverride = FoodPreferability.Undefined, float? minNutrition = null, bool allowVenerated = false)
	{
		foodDef = null;
		...
		if (getter.RaceProps.Humanlike || getter.IsColonyMechPlayerControlled)
		{
			...
			if (allowHarvest && getterCanManipulate)
			{
				Thing thing = GenClosest.ClosestThingReachable(searchRegionsMax: (!forceScanWholeMap || bestThing != null) ? 30 : (-1), root: getter.Position, map: getter.Map, thingReq: ThingRequest.ForGroup(ThingRequestGroup.HarvestablePlant), peMode: PathEndMode.Touch, traverseParams: TraverseParms.For(getter), maxDistance: 9999f, validator: delegate(Thing x)
				{
					...
					if (!eater.WillEat_NewTemp(harvestedThingDef, getter, careIfNotAcceptableForTitle: true, allowVenerated))
					{
						return false;
					}
		...
		else
		{
			...
			{
				if (item is Pawn pawn && pawn != getter && pawn.RaceProps.Animal && pawn.CurJob != null && pawn.CurJob.def == JobDefOf.Ingest && pawn.CurJob.GetTarget(TargetIndex.A).HasThing)
				{
	...
	{
		if (!(Find.Selector.SingleSelectedThing is Pawn pawn) || !TryFindBestFoodSourceFor_NewTemp(pawn, pawn, desperate: true, out var foodSource, out var _, canRefillDispenser: false, canUseInventory: false))
		{
			return;
		}
	...
	{
		if ((inv.def.IsNutritionGivingIngestible && pawn.WillEat_NewTemp(inv)) || (inv.def.IsNonMedicalDrug && pawn.CanTakeDrug(inv.def)))
		{

...
public static class BillUtility
{
	...
	{
		if (bill.billStack.billGiver is Building_MechGestator building_MechGestator)
		{
	...
	private static void LayoutTooltip(RecipeDef recipe, bool draw)
	{
		...
		if (!recipe.ingredients.NullOrEmpty())
		{
			...
			foreach (IngredientCount ingredient in recipe.ingredients)
			{
				...
				recipeTooltipLayout.Newline();
				DisplayIngredientIconRow(enumerable2, draw, ingredient.CountFor(recipe));
			}
		}
	...
	private static void DisplayIngredientIconRow(IEnumerable<TextureAndColor> icons, bool draw, float? count = null)
	{

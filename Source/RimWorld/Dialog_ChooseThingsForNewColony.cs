...
public class Dialog_ChooseThingsForNewColony : Window
{
	...
	public int SlaveCount
	{
		get
		{
			...
			for (int i = 0; i < colonists.Count; i++)
			{
				if (selected.Contains(colonists[i]) && colonists[i] is Pawn pawn && pawn.IsSlaveOfColony)
				{
					num++;
				}
			}
	...
	public override void PostOpen()
	{
		...
		{
			if (item2 is Precept_Relic precept_Relic && precept_Relic.RelicInPlayerPossession)
			{
	...
	private void ConfirmArchonexusSettlementConsequences(List<Thing> selectedList, bool onlySlavesSelected)
	{
		...
		void CloseAction()
		{
			...
			if (onlySlavesSelected)
			{
				...
				{
					if (selected is Pawn pawn && pawn.IsSlaveOfColony)
					{

...
public class CompShuttle : ThingComp
{
	...
	private int ContainedColonistCount
	{
		get
		{
			...
			{
				if (innerContainer[i] is Pawn pawn && pawn.IsFreeColonist)
				{
					num++;
				}
			}
	...
	public void SendLaunchedSignals()
	{
		...
		for (int i = 0; i < transportersInGroup.Count; i++)
		{
			...
			{
				if (transportersInGroup[i].innerContainer[j] is Pawn pawn && pawn.IsColonist && !requiredPawns.Contains(pawn))
				{
					list.Add(pawn);
				}
			}
	...
	public virtual bool IsAllowedNow(Thing t)
	{
		...
		{
			if (item != t && item is Pawn pawn && pawn.IsColonist)
			{
				num++;
			}
		}

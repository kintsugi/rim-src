...
public static class QuestUtility
{
	...
	public static int GetQuestTicksRemaining(Quest quest)
	{
		...
		{
			if (item is QuestPart_Delay questPart_Delay && questPart_Delay.State == QuestPartState.Enabled && questPart_Delay.isBad && !questPart_Delay.expiryInfoPart.NullOrEmpty())
			{
	...
	public static void AppendInspectStringsFromQuestParts(Action<string, Quest> func, ISelectable target, out int count)
	{
		...
		for (int i = 0; i < questsListForReading.Count; i++)
		{
			...
			{
				if (tmpQuestParts[j] is QuestPartActivable questPartActivable && questPartActivable.State == QuestPartState.Enabled)
				{
	...
	public static IEnumerable<Gizmo> GetQuestRelatedGizmos(Thing thing)
	{
		...
		for (int i = 0; i < quests.Count; i++)
		{
			...
			{
				if (!(parts[j] is QuestPartActivable questPartActivable) || questPartActivable.State != QuestPartState.Enabled)
				{
					continue;
				}
	...
	public static bool AnyQuestDisablesRandomMoodCausedMentalBreaksFor(Pawn p)
	{
		...
		for (int i = 0; i < questsListForReading.Count; i++)
		{
			...
			for (int j = 0; j < partsListForReading.Count; j++)
			{
				if (partsListForReading[j] is QuestPart_DisableRandomMoodCausedMentalBreaks questPart_DisableRandomMoodCausedMentalBreaks && questPart_DisableRandomMoodCausedMentalBreaks.State == QuestPartState.Enabled && questPart_DisableRandomMoodCausedMentalBreaks.pawns.Contains(p))
				{
					return true;
				}
			}
	...
	public static bool IsGoodwillLockedByQuest(Faction a, Faction b)
	{
		...
		for (int i = 0; i < questsListForReading.Count; i++)
		{
			...
			for (int j = 0; j < partsListForReading.Count; j++)
			{
				if (partsListForReading[j] is QuestPart_FactionGoodwillLocked questPart_FactionGoodwillLocked && questPart_FactionGoodwillLocked.State == QuestPartState.Enabled && questPart_FactionGoodwillLocked.AppliesTo(a, b))
				{
					return true;
				}
			}

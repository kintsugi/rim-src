...
public class TransportShip : IExposable, ILoadReferenceable
{
	...
	private bool CanDispose
	{
		get
		{
			if (!started || curJob != null || shipThing.Spawned || shipJobs.Any((ShipJob x) => x.def.blocksDisposalIfQueuedUnspawned) || Find.World.worldObjects.TravelingTransportPods.Any((TravelingTransportPods x) => x.arrivalAction is TransportPodsArrivalAction_TransportShip transportPodsArrivalAction_TransportShip && transportPodsArrivalAction_TransportShip.transportShip == this) || Find.QuestManager.IsReservedByAnyQuest(this))
			{
				return false;
			}
			return true;

...
public class GenStep_ScatterMechanoidRemains : GenStep_Scatterer
{
	...
	protected override void ScatterAt(IntVec3 loc, Map map, GenStepParams parms, int count = 1)
	{
		...
		foreach (IntVec3 item2 in cellRect.InRandomOrder())
		{
			...
			ThingDef ancientMechDetritus = ThingDefOf.AncientMechDetritus;
			Rot4 rot = (ancientMechDetritus.rotatable ? Rot4.Random : Rot4.North);
			bool flag = true;
			foreach (IntVec3 item3 in GenAdj.OccupiedRect(item2, rot, ancientMechDetritus.size))
			{
			...
			if (flag)
			{
				GenSpawn.Spawn(ThingMaker.MakeThing(ancientMechDetritus), item2, map, rot);

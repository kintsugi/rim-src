...
public static class PlayerItemAccessibilityUtility
{
	...
	private static void CacheAccessibleThings(int nearTile)
	{
		...
		{
			if (cachedAccessibleThings[num8] is Pawn pawn2 && pawn2.IsFreeColonist && !pawn2.Dead && !pawn2.Downed && pawn2.workSettings.WorkIsActive(WorkTypeDefOf.Crafting))
			{
				num7++;
			}
		}
		if (num7 > 0)
		{
			...
			{
				if (!(cachedAccessibleThings[num9] is Building_WorkTable building_WorkTable) || !building_WorkTable.Spawned || !tmpWorkTables.Add(building_WorkTable.def))
				{
					continue;
				}

...
public class Dialog_StyleSelection : Window
{
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		for (int i = 0; i < Find.IdeoManager.selectedStyleCategories.Count; i++)
		{
			...
			if (Widgets.ButtonImage(rect, cat2.Icon, baseColor))
			{
			...
			if (Mouse.IsOver(rect))
			{
				...
				TooltipHandler.TipRegion(rect, IdeoUIUtility.StyleTooltip(cat2, IdeoEditMode.GameStart, null, tmpStyles, skipDominanceDesc: true));
			}
			num += 28f;
		}

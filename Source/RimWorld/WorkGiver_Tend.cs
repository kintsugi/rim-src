...
public class WorkGiver_Tend : WorkGiver_Scanner
{
	...
	{
		if (!(t is Pawn pawn2) || pawn.WorkTypeIsDisabled(WorkTypeDefOf.Doctor) || (def.tendToHumanlikesOnly && !pawn2.RaceProps.Humanlike) || (def.tendToAnimalsOnly && !pawn2.RaceProps.Animal) || !GoodLayingStatusForTend(pawn2, pawn) || !HealthAIUtility.ShouldBeTendedNowByPlayer(pawn2) || pawn2.IsForbidden(pawn) || !pawn.CanReserve(pawn2, 1, -1, null, forced) || (pawn2.InAggroMentalState && !pawn2.health.hediffSet.HasHediff(HediffDefOf.Scaria)))
		{
			return false;
		}
		return true;

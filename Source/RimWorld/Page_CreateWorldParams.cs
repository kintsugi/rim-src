...
public class Page_CreateWorldParams : Page
{
	...
	public override void DoWindowContents(Rect rect)
	{
		...
		rainfall = (OverallRainfall)Mathf.RoundToInt(Widgets.HorizontalSlider_NewTemp(rect5, (float)rainfall, 0f, OverallRainfallUtility.EnumValuesCount - 1, middleAlignment: true, "PlanetRainfall_Normal".Translate(), "PlanetRainfall_Low".Translate(), "PlanetRainfall_High".Translate(), 1f));
		num2 += 40f;
		Widgets.Label(new Rect(0f, num2, 200f, 30f), "PlanetTemperature".Translate());
		Rect rect6 = new Rect(200f, num2, width, 30f);
		temperature = (OverallTemperature)Mathf.RoundToInt(Widgets.HorizontalSlider_NewTemp(rect6, (float)temperature, 0f, OverallTemperatureUtility.EnumValuesCount - 1, middleAlignment: true, "PlanetTemperature_Normal".Translate(), "PlanetTemperature_Low".Translate(), "PlanetTemperature_High".Translate(), 1f));
		num2 += 40f;
		Widgets.Label(new Rect(0f, num2, 200f, 30f), "PlanetPopulation".Translate());
		Rect rect7 = new Rect(200f, num2, width, 30f);
		population = (OverallPopulation)Mathf.RoundToInt(Widgets.HorizontalSlider_NewTemp(rect7, (float)population, 0f, OverallPopulationUtility.EnumValuesCount - 1, middleAlignment: true, "PlanetPopulation_Normal".Translate(), "PlanetPopulation_Low".Translate(), "PlanetPopulation_High".Translate(), 1f));
		if (ModsConfig.BiotechActive)
		{
			num2 += 40f;
			Widgets.Label(new Rect(0f, num2, 200f, 30f), "PlanetPollution".Translate());
			Rect rect8 = new Rect(200f, num2, width, 30f);
			pollution = Widgets.HorizontalSlider_NewTemp(rect8, pollution, 0f, 1f, middleAlignment: true, pollution.ToStringPercent(), null, null, 0.05f);
		}
		Widgets.EndGroup();

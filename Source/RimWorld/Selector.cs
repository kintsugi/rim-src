...
public class Selector
{
		...
		else
		{
			...
			if (obj3 != null)
			{
				if (!ShiftIsHeld)
				{
					...
					{
						if (x is Thing thing3 && thing3.def.category != ThingCategory.Item)
						{
					...
					}))
					{
						...
						else if (list[list.Count - 1] is Building building && building.MaxItemsInCell > 1)
						{
							ClearSelection();
			...
			if (list.Count >= 2 && list[0] is Thing thing2 && thing2.def.category == ThingCategory.Item && thing2.Spawned)
			{
				object obj4 = list.FirstOrDefault((object x) => x is Building building2 && building2.MaxItemsInCell > 1);
				if (obj4 != null)
				{
					Select(obj4);
				}
	...
	private void SelectAllMatchingObjectUnderMouseOnScreen()
	{
		...
		Predicate<Thing> predicate = delegate(Thing t)
		{
			...
			if (innerIfMinified is Pawn pawn)
			{

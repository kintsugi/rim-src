...
public abstract class WorkGiver_Warden : WorkGiver_Scanner
{
	...
	{
		if (!(prisoner is Pawn pawn) || !pawn.IsPrisonerOfColony || !pawn.guest.PrisonerIsSecure || !pawn.Spawned || pawn.InAggroMentalState || prisoner.IsForbidden(warden) || pawn.IsFormingCaravan() || !warden.CanReserveAndReach(pawn, PathEndMode.OnCell, warden.NormalMaxDanger(), 1, -1, null, forced))
		{
			return false;
		}
		return true;
	...
	{
		if (!(slave is Pawn pawn) || !pawn.IsSlaveOfColony || !pawn.guest.SlaveIsSecure || !pawn.Spawned || pawn.InAggroMentalState || pawn.IsForbidden(warden) || pawn.IsFormingCaravan() || !warden.CanReserveAndReach(pawn, PathEndMode.Touch, warden.NormalMaxDanger(), 1, -1, null, forced))
		{
			return false;
		}
		return true;

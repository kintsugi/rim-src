	...
	public bool WillCloseSoon
	{
		get
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				{
					if (thingList[j] is Pawn pawn && !pawn.HostileTo(this) && !pawn.Downed && (pawn.Position == base.Position || (pawn.pather.Moving && pawn.pather.nextCell == base.Position)))
					{
						return true;
					}
				}
	...
	private static int AlignQualityAgainst(IntVec3 c, IntVec3 offset, Map map)
	{
		...
		for (int i = 0; i < thingList.Count; i++)
		{
			...
			if (thing2 != null)
			{
				...
				if (thing2.def.entityDefToBuild is ThingDef thingDef && thingDef.IsFence)
				{
					return 1;
				}
				if (typeof(Building_Door).IsAssignableFrom(thing.def.thingClass))

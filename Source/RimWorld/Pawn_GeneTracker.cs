...
public class Pawn_GeneTracker : IExposable
{
	...
	private void Notify_GenesChanged(GeneDef addedOrRemovedGene)
	{
		...
		{
			if (SelectGene((Gene x) => x.def.graphicData != null && x.def.graphicData.skinIsHairColor, out var chosen2))
			{
	...
	public void Notify_PawnDied()
	{
		...
		{
			genesListForReading[num].Notify_PawnDied();
		}
	}
	...
	{
		Debug_AddAllGenes_NewTemp(xenogene: true);
	}

	public void Debug_AddAllGenes_NewTemp(bool xenogene)
	{
		if (!ModsConfig.BiotechActive)
		{
			return;

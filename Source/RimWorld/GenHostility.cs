...
public static class GenHostility
{
	...
	private static Pawn GetPreyOfMyFaction(Pawn predator, Faction myFaction)
	{
		...
		if (curJob != null && curJob.def == JobDefOf.PredatorHunt && !predator.jobs.curDriver.ended && curJob.GetTarget(TargetIndex.A).Thing is Pawn pawn && !pawn.Dead && pawn.Faction == myFaction)
		{
			return pawn;
		}
		return null;

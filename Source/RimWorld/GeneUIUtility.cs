...
public static class GeneUIUtility
{
	...
	private static void DrawGeneSections(Rect rect, Thing target, GeneSet genesOverride, ref Vector2 scrollPosition)
	{
		...
		else
		{
			...
			{
				DrawGeneDef_NewTemp(geneDefs[i], r, geneType);
			}, containingRect);
		}
		if (Event.current.type == EventType.Layout)
		{
	...
	private static void DoDebugButton(Rect buttonRect, Thing target, GeneSet genesOverride)
	{
		...
		if (sourcePawn != null)
		{
			...
			{
				sourcePawn.genes.Debug_AddAllGenes_NewTemp(xenogene: true);
			}));
			list.Add(new FloatMenuOption("Add all genes (endogene)", delegate
			{
				sourcePawn.genes.Debug_AddAllGenes_NewTemp(xenogene: false);
			}));
	...
	{
		DrawGeneDef_NewTemp(gene, geneRect, geneType, () => extraTooltip, doBackground, clickable, overridden);
	}

	public static void DrawGeneDef_NewTemp(GeneDef gene, Rect geneRect, GeneType geneType, Func<string> extraTooltip = null, bool doBackground = true, bool clickable = true, bool overridden = false)
	{

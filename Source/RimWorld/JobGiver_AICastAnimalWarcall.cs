...
public class JobGiver_AICastAnimalWarcall : JobGiver_AICastAbility
{
	...
	protected override LocalTargetInfo GetTarget(Pawn caster, Ability ability)
	{
		...
		{
			if (item.RaceProps.Animal && item.Faction == null && item.MentalStateDef != MentalStateDefOf.BerserkWarcall && item.Position.InHorDistOf(caster.Position, 30f) && ability.CanApplyOn(new LocalTargetInfo(item)))
			{

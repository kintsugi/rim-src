...
public class Pawn_StyleObserverTracker : IExposable
{
	...
	public void StyleObserverTick()
	{
		...
		if (pawn.Awake() && !PawnUtility.IsBiologicallyOrArtificiallyBlind(pawn))
		{
			Room room = pawn.GetRoom();
			if (room != null && CellFinder.TryFindRandomCellNear(pawn.Position, pawn.Map, 5, (IntVec3 x) => CellValid(x), out var result))
			{

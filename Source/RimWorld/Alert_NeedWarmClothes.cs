...
public class Alert_NeedWarmClothes : Alert
{
	...
	private static Comparison<Thing> SortByInsulationCold = (Thing a, Thing b) => a.GetStatValue(StatDefOf.Insulation_Cold, applyPostProcess: true, 1).CompareTo(b.GetStatValue(StatDefOf.Insulation_Cold, applyPostProcess: true, 1));
	...
	private int NeededWarmClothesCount(Map map)
	{
		...
		{
			if ((freeColonist.Spawned || freeColonist.BrieflyDespawned()) && !freeColonist.DevelopmentalStage.Baby())
			{
				num++;
			}
		}
	...
	private int ColonistsWithWarmClothesCount(Map map)
	{
		...
		foreach (Pawn freeColonist in map.mapPawns.FreeColonists)
		{
			if ((freeColonist.Spawned || freeColonist.BrieflyDespawned()) && !freeColonist.DevelopmentalStage.Baby() && freeColonist.GetStatValue(StatDefOf.ComfyTemperatureMin) <= num)
			{
				num2++;
			}
		}

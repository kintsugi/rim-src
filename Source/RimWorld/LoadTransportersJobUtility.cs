...
public static class LoadTransportersJobUtility
{
	...
	public static ThingCount FindThingToLoad(Pawn p, CompTransporter transporter)
	{
		...
		if (thing == null)
		{
			...
			{
				if (neededThing is Pawn pawn && ((!pawn.IsColonist && !pawn.IsColonyMech) || pawn.Downed) && !pawn.inventory.UnloadEverything && p.CanReserveAndReach(pawn, PathEndMode.Touch, Danger.Deadly))
				{
					neededThings.Clear();
					tmpAlreadyLoading.Clear();

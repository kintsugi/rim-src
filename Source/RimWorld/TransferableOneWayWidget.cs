...
public class TransferableOneWayWidget
{
	...
	{
		if (ModsConfig.BiotechActive && trad.HasAnyThing && trad.AnyThing is Pawn pawn && !pawn.Dead && pawn.needs.energy != null)
		{
			Widgets.DrawHighlightIfMouseover(rect);
	...
	{
		if (trad.HasAnyThing && trad.AnyThing is Pawn pawn && pawn.equipment != null && pawn.equipment.Primary != null)
		{
	...
	private float GetMass(Thing thing)
	{
		...
		else if (ignoreSpawnedCorpseGearAndInventoryMass && thing is Corpse corpse && corpse.Spawned)
		{

...
public class CompAbilityEffect_Farskip : CompAbilityEffect
{
	...
	private IEnumerable<Pawn> PawnsToSkip()
	{
		...
		{
			if (item is Pawn pawn && !pawn.Dead && (pawn.IsColonist || pawn.IsPrisonerOfColony || (!homeMap && pawn.RaceProps.Animal && pawn.Faction != null && pawn.Faction.IsPlayer)))
			{
				yield return pawn;
			}
		}
	...
	{
		if (target.WorldObject is MapParent mapParent && mapParent.Map != null && AlliedPawnOnMap(mapParent.Map) == null)
		{
			return false;
		}

...
public class Dialog_AutoSlaughter : Window
{
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		DoAnimalHeader(map, listing_Standard.GetRect(24f), listing_Standard.GetRect(24f));
		listing_Standard.Gap(6f);
		_ = map.autoSlaughterManager.configs;
		int num = 0;
		foreach (AutoSlaughterConfig item in configsOrdered)
		{
			...
			{
				DoAnimalRow(map, rect2, item, num);
			}
			listing_Standard.Gap(6f);
			num++;
		}
	...
	private void DoAnimalRow(Map map, Rect rect, AutoSlaughterConfig config, int index)
	{
		...
		row.Label(config.animal.LabelCap.Truncate(width), width, config.animal.LabelCap);
		GUI.color = color;

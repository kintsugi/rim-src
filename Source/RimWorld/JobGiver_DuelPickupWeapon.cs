	...
	protected override Job TryGiveJob(Pawn pawn)
	{
		...
		for (int i = 0; i < GenRadial.NumCellsInRadius(scanRadius); i++)
		{
			...
			foreach (Thing thing2 in c.GetThingList(selectedTarget.Map))
			{
				...
				if (thing2.def.IsMeleeWeapon && pawn.CanReserveAndReach(thing2, PathEndMode.ClosestTouch, Danger.Deadly) && num2 < pawn.Position.DistanceTo(pawn2.Position) && num2 < pawn2.Position.DistanceTo(thing2.Position) && num > num2)
				{
					thing = thing2;
					num = num2;
				}

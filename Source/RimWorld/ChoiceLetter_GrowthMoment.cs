...
public class ChoiceLetter_GrowthMoment : LetterWithTimeout
{
	...
	{
		return DefDatabase<SkillDef>.AllDefsListForReading.Where((SkillDef s) => IsValidGrowthPassionOption_NewTemp(pawn, s, checkGenes: false)).InRandomOrder().Take(count)
			.ToList();
	}

...
public class TraitDegreeData
{
	...
	public List<IssueDef> GetAffectedIssues(TraitDef def)
	{
		if (affectedIssuesCached == null)
		{
			...
			{
				if (!affectedIssuesCached.Contains(allDefsListForReading[i].issue) && allDefsListForReading[i].TraitsAffecting.Any((TraitRequirement x) => x.def == def && (x.degree.HasValue ? x.degree.Value : 0) == degree))
				{
	...
	public List<MemeDef> GetAffectedMemes(TraitDef def, bool agreeable)
	{
		if (agreeable)
		{
			if (agreeableMemesCached == null)
			{
				...
				for (int i = 0; i < allDefsListForReading.Count; i++)
				{
					if (!allDefsListForReading[i].agreeableTraits.NullOrEmpty() && allDefsListForReading[i].agreeableTraits.Any((TraitRequirement x) => x.def == def && (x.degree.HasValue ? x.degree.Value : 0) == degree))
					{
		...
		if (disagreeableMemesCached == null)
		{
			...
			{
				if (!allDefsListForReading2[j].disagreeableTraits.NullOrEmpty() && allDefsListForReading2[j].disagreeableTraits.Any((TraitRequirement x) => x.def == def && (x.degree.HasValue ? x.degree.Value : 0) == degree))
				{

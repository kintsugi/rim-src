...
public class Dialog_FormCaravan : Window
{
	...
	private bool ShowCancelButton
	{
		get
		{
			...
			{
				if (transferables[i].AnyThing is Pawn pawn && pawn.IsColonist && !pawn.Downed)
				{
					flag = true;
					break;
				}
	...
	private void AddItemsFromTransferablesToRandomInventories(List<Pawn> pawns)
	{
		...
		for (int l = 0; l < transferables.Count; l++)
		{
			...
			{
				if (AutoStripSpawnedCorpses && originalThing is Corpse corpse && corpse.Spawned)
				{
					corpse.Strip();
				}
	...
	private void Notify_TransferablesChanged()
	{
		...
		if (ModsConfig.BiotechActive)
		{
			foreach (TransferableOneWay transferable in transferables)
			{
				...
				{
					if (!transferable.IsThing || !(transferable.things[i] is Pawn pawn) || !pawn.IsColonyMech)
					{
						continue;
					}
					Pawn overseer = pawn.GetOverseer();

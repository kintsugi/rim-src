...
public static class PawnUtility
{
	...
	public static bool IsInteractionBlocked(this Pawn pawn, InteractionDef interaction, bool isInitiator, bool isRandom)
	{
		...
		{
			RitualRole ritualRole = lordJob_Ritual.RoleFor(pawn, includeForced: true);
			if (ritualRole != null && ritualRole.blocksSocialInteraction)
			{
				return true;
			}
		}
	...
	{
		PawnPosture pawnPosture = (p.Dead ? PawnPosture.LayingOnGroundNormal : ((!ModsConfig.BiotechActive || !p.IsCharging()) ? (p.Downed ? ((p.jobs == null || !p.jobs.posture.Laying()) ? PawnPosture.LayingOnGroundNormal : p.jobs.posture) : ((!(p.ParentHolder is IThingHolderWithDrawnPawn thingHolderWithDrawnPawn)) ? ((p.jobs != null) ? p.jobs.posture : PawnPosture.Standing) : thingHolderWithDrawnPawn.HeldPawnPosture)) : PawnPosture.Standing));
	...
	public static Pawn PawnBlockingPathAt(IntVec3 c, Pawn forPawn, bool actAsIfHadCollideWithPawnsJob = false, bool collideOnlyWithStandingPawns = false, bool forPathFinder = false)
	{
		...
		{
			if (!(thingList[i] is Pawn pawn) || pawn == forPawn || pawn.Downed || (collideOnlyWithStandingPawns && (pawn.pather.MovingNow || (pawn.pather.Moving && pawn.pather.MovedRecently(60)))) || PawnsCanShareCellBecauseOfBodySize(pawn, forPawn))
			{
				continue;
			}
	...
	public static bool CanTakeDrug(this Pawn pawn, ThingDef drug)
	{
		if (pawn.IsTeetotaler())
		{
			...
			if (ModsConfig.BiotechActive && pawn.genes != null)
			{
				...
				{
					if (item is Gene_ChemicalDependency gene_ChemicalDependency && gene_ChemicalDependency.def.chemical == compProperties.chemical)
					{
						return true;
					}
				}
	...
	{
		if (ModsConfig.IdeologyActive && t is Corpse corpse && corpse.MapHeld != null)
		{
	...
	public static bool DutyActiveWhenDown(this Pawn pawn, bool onlyInBed = false)
	{
		...
		if (pawn.GetLord()?.LordJob is LordJob_Ritual lordJob_Ritual && (ritualRole = lordJob_Ritual.assignments?.RoleForPawn(pawn)) != null && ritualRole.allowDowned)
		{
			return true;
		}
		return false;

...
public class WorkGiver_Warden_DeliverFood : WorkGiver_Warden
{
	...
	{
		if (foodSource.def.IsNutritionGivingIngestible && p.WillEat_NewTemp(foodSource))
		{
			return foodSource.GetStatValue(StatDefOf.Nutrition) * (float)foodSource.stackCount;
		}
		if (p.RaceProps.ToolUser && p.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation) && foodSource is Building_NutrientPasteDispenser building_NutrientPasteDispenser && building_NutrientPasteDispenser.CanDispenseNow && p.CanReach(building_NutrientPasteDispenser.InteractionCell, PathEndMode.OnCell, Danger.Some))
		{
			return 99999f;
		}
		return 0f;

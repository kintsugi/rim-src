...
public class MainTabWindow_Quests : MainTabWindow
{
	...
	private void DoRightAlignedInfo(Rect innerRect, ref float curY, float curYBeforeAcceptButton)
	{
		...
		if (selected.State == QuestState.Ongoing)
		{
			...
			{
				if (!(tmpQuestParts[i] is QuestPartActivable questPartActivable) || questPartActivable.State != QuestPartState.Enabled)
				{
					continue;
				}
	...
	private void DoRewards(Rect innerRect, ref float curY)
	{
		...
		for (int j = 0; j < choice.choices.Count; j++)
		{
			...
			if (num > 0f && (choice.choices[j].rewards.Count != 1 || !(choice.choices[j].rewards[0] is Reward_Items reward_Items) || reward_Items.items == null || reward_Items.items.Count != 1 || !(reward_Items.items[0].StyleSourcePrecept is Precept_Relic)))
			{
	...
	private void DoDebugInfo(Rect innerRect, ref float curY)
	{
		...
		{
			if (partsListForReading[j] is QuestPartActivable questPartActivable && questPartActivable.State == QuestPartState.Enabled)
			{

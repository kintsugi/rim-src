...
public class StatWorker
{
	...
	public virtual float GetValueUnfinalized(StatRequest req, bool applyPostProcess = true)
	{
		...
		if (pawn != null)
		{
			...
			if (ModsConfig.BiotechActive && pawn.genes != null)
			{
				...
				for (int num3 = 0; num3 < genesListForReading.Count; num3++)
				{
					...
					num += genesListForReading[num3].def.statOffsets.GetStatOffsetFromList(stat);
					...
					{
						ConditionalStatAffecter conditionalStatAffecter2 = genesListForReading[num3].def.conditionalStatAffecters[num4];
			...
			{
				for (int num5 = 0; num5 < pawn.apparel.WornApparel.Count; num5++)
				{
					num += StatOffsetFromGear(pawn.apparel.WornApparel[num5], stat);
				}
			}
			...
			if (pawn.story != null)
			{
				for (int num6 = 0; num6 < pawn.story.traits.allTraits.Count; num6++)
				{
					if (!pawn.story.traits.allTraits[num6].Suppressed)
					{
						num *= pawn.story.traits.allTraits[num6].MultiplierOfStat(stat);
					}
				}
			}
			for (int num7 = 0; num7 < hediffs.Count; num7++)
			{
				HediffStage curStage2 = hediffs[num7].CurStage;
				if (curStage2 != null)
				{
					float num8 = curStage2.statFactors.GetStatFactorFromList(stat);
					if (Math.Abs(num8 - 1f) > float.Epsilon && curStage2.statFactorEffectMultiplier != null)
					{
						num8 = ScaleFactor(num8, pawn.GetStatValue(curStage2.statFactorEffectMultiplier));
					}
					if (curStage2.multiplyStatChangesBySeverity)
					{
						num8 = ScaleFactor(num8, hediffs[num7].Severity);
					}
					num *= num8;
				}
			}
			if (pawn.Ideo != null)
			{
				...
				for (int num9 = 0; num9 < preceptsListForReading2.Count; num9++)
				{
					if (preceptsListForReading2[num9].def.statFactors != null)
					{
						float statFactorFromList = preceptsListForReading2[num9].def.statFactors.GetStatFactorFromList(stat);
						num *= statFactorFromList;
					}
					if (preceptsListForReading2[num9].def.conditionalStatAffecters == null)
					{
						continue;
					}
					for (int num10 = 0; num10 < preceptsListForReading2[num9].def.conditionalStatAffecters.Count; num10++)
					{
						ConditionalStatAffecter conditionalStatAffecter3 = preceptsListForReading2[num9].def.conditionalStatAffecters[num10];
						if (conditionalStatAffecter3.statFactors != null && conditionalStatAffecter3.Applies(req))
						{
							float statFactorFromList2 = conditionalStatAffecter3.statFactors.GetStatFactorFromList(stat);
			...
			if (ModsConfig.BiotechActive && pawn.genes != null)
			{
				...
				for (int num11 = 0; num11 < genesListForReading2.Count; num11++)
				{
					if (!genesListForReading2[num11].Active)
					{
						continue;
					}
					num *= genesListForReading2[num11].def.statFactors.GetStatFactorFromList(stat);
					if (genesListForReading2[num11].def.conditionalStatAffecters == null)
					{
						continue;
					}
					for (int num12 = 0; num12 < genesListForReading2[num11].def.conditionalStatAffecters.Count; num12++)
					{
						ConditionalStatAffecter conditionalStatAffecter4 = genesListForReading2[num11].def.conditionalStatAffecters[num12];
		...
		if (req.ForAbility)
		{
			if (stat.statFactors != null)
			{
				...
				{
					num *= req.AbilityDef.statBases.GetStatValueFromList(stat.statFactors[num13], 1f);
				}
			}
			Pawn pawn2 = req.Pawn;
			if (pawn2 != null && pawn2.Ideo != null)
			{
				...
				for (int num14 = 0; num14 < preceptsListForReading3.Count; num14++)
				{
					if (preceptsListForReading3[num14].def.statFactors != null)
					{
						float statFactorFromList3 = preceptsListForReading3[num14].def.statFactors.GetStatFactorFromList(stat);
						num *= statFactorFromList3;
					}
					if (preceptsListForReading3[num14].def.abilityStatFactors == null)
					{
						continue;
					}
					foreach (AbilityStatModifiers abilityStatFactor in preceptsListForReading3[num14].def.abilityStatFactors)
					{
						if (abilityStatFactor.ability == req.AbilityDef)
						{
							float statFactorFromList4 = abilityStatFactor.modifiers.GetStatFactorFromList(stat);
							num *= statFactorFromList4;
						}
					}
				}
			}
	...
	public virtual string GetExplanationUnfinalized(StatRequest req, ToStringNumberSense numberSense)
	{
		...
		if (stat.statFactors != null)
		{
			...
			{
				StatDef statDef = stat.statFactors[num6];
		...
		if (pawn != null)
		{
			if (pawn.skills != null)
			{
				if (stat.skillNeedFactors != null)
				{
					...
					{
						SkillNeed skillNeed2 = stat.skillNeedFactors[num7];
	...
	public virtual bool IsDisabledFor(Thing thing)
	{
		...
		if (thing is Pawn pawn && pawn.story != null)
		{
			if (stat.skillNeedFactors != null)
			{
	...
	{
		if (!(statRequest.Thing is Pawn pawn))
		{
			yield break;
		}
		List<Hediff> hediffs = pawn.health.hediffSet.hediffs;
	...
	private static bool DisplayTradeStats(StatRequest req)
	{
		...
		if (ModsConfig.BiotechActive && req.HasThing && req.Thing is Pawn pawn && pawn.IsColonyMech)
		{
			return true;
		}

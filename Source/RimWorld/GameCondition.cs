...
public class GameCondition : IExposable, ILoadReferenceable
{
	...
	public virtual string TooltipString
	{
		get
		{
			...
			{
				text += def.jumpToSourceKey.Translate();
			}
			else if (quest != null)
			{
				text += "CausedByQuest".Translate(quest.name);
			}
			else if (!def.natural)
			{
	...
	public bool HiddenByOtherCondition
	{
		get
		{
			...
			{
				if (gameConditionManager.ConditionIsActive(def.silencedByConditions[i]))
				{
					return true;
				}
			}
	...
	{
		if (!suppressEndMessage && def.endMessage != null && !HiddenByOtherCondition)
		{

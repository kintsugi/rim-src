...
public class JobGiver_DropUnusedInventory : ThinkNode_JobGiver
{
	...
	{
		if (drug.def.IsDrug && pawn.drugs != null && !pawn.drugs.AllowedToTakeScheduledEver(drug.def) && pawn.drugs.CurrentPolicy[drug.def].takeToInventory == 0 && !AddictionUtility.IsAddicted(pawn, drug))
		{
			return false;
		}
		return true;

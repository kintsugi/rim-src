...
public class RitualOutcomeEffectWorker_FromQuality : RitualOutcomeEffectWorker
{
	...
	{
		return def.outcomeChances.Where((OutcomeChance o) => OutcomePossible(o, ritual)).RandomElementByWeight((OutcomeChance c) => ChanceWithQuality(c, quality));
	}

	public virtual float GetOutcomeChanceAtQuality(LordJob_Ritual ritual, OutcomeChance outcome, float quality)
	{
		float num = def.outcomeChances.Where((OutcomeChance o) => OutcomePossible(o, ritual)).Sum((OutcomeChance c) => ChanceWithQuality(c, quality));
		return ChanceWithQuality(outcome, quality) / num;
	}

	protected static float ChanceWithQuality(OutcomeChance outcome, float quality)
	{
		if (!outcome.Positive)
		{
	...
	protected virtual void ApplyDevelopmentPoints(Precept_Ritual ritual, OutcomeChance outcome, out string extraOutcomeDesc)
	{

...
public class FactionManager : IExposable
{
	...
	private bool FactionCanBeRemoved(Faction faction)
	{
		...
		return true;
		bool CheckPawns(List<Pawn> pawns)
		{
			for (int i = 0; i < pawns.Count; i++)
			{
				Pawn pawn = pawns[i];
				if (!pawn.Dead && ((pawn.Faction != null && pawn.Faction == faction) || faction == pawn.GetExtraHomeFaction() || faction == pawn.GetExtraMiniFaction() || faction == pawn.SlaveFaction))
				{
					return false;
				}
			}

...
public class Precept : IExposable, ILoadReferenceable
{
	...
	public virtual string GetTip()
	{
		...
		for (int j = 0; j < def.comps.Count; j++)
		{
			...
			else if (def.comps[j] is PreceptComp_SituationalThought preceptComp_SituationalThought && preceptComp_SituationalThought.AffectsMood)
			{
				preceptComp = preceptComp_SituationalThought;
			}
			else if (def.comps[j] is PreceptComp_KnowsMemoryThought preceptComp_KnowsMemoryThought && preceptComp_KnowsMemoryThought.AffectsMood)
			{
				preceptComp = preceptComp_KnowsMemoryThought;
			}
			else
			{
				if (!(def.comps[j] is PreceptComp_BedThought preceptComp_BedThought) || !preceptComp_BedThought.AffectsMood)
				{
					continue;
				}
		...
		for (int k = 0; k < def.comps.Count; k++)
		{
			...
			if (def.comps[k] is PreceptComp_SituationalThought preceptComp_SituationalThought2 && !preceptComp_SituationalThought2.AffectsMood)
			{
				preceptComp2 = preceptComp_SituationalThought2;
			}
			else
			{
				if (!(def.comps[k] is PreceptComp_KnowsMemoryThought preceptComp_KnowsMemoryThought2) || preceptComp_KnowsMemoryThought2.AffectsMood)
				{
					continue;
				}
		...
		{
			for (int m = 0; m < def.conditionalStatAffecters.Count; m++)
			{
				ConditionalStatAffecter conditionalStatAffecter = def.conditionalStatAffecters[m];
				if (conditionalStatAffecter.statOffsets != null)
				{
					for (int n = 0; n < conditionalStatAffecter.statOffsets.Count; n++)
					{
						TryDrawStatLine();
						tmpCompsDesc.AppendInNewLine("  - " + conditionalStatAffecter.statOffsets[n].stat.LabelCap + ": " + conditionalStatAffecter.statOffsets[n].ValueToStringAsOffset + " (" + conditionalStatAffecter.Label + ")");
					}
				}
			}
		}
		...
		{
			for (int num = 0; num < def.statFactors.Count; num++)
			{
				TryDrawStatLine();
				tmpCompsDesc.AppendInNewLine("  - " + def.statFactors[num].stat.LabelCap + ": " + def.statFactors[num].ToStringAsFactor);
			}
		}
		...
		if (def.conditionalStatAffecters != null)
		{
			for (int num2 = 0; num2 < def.conditionalStatAffecters.Count; num2++)
			{
				ConditionalStatAffecter conditionalStatAffecter2 = def.conditionalStatAffecters[num2];
				if (conditionalStatAffecter2.statFactors != null)
				{
					for (int num3 = 0; num3 < conditionalStatAffecter2.statFactors.Count; num3++)
					{
						TryDrawStatLine();
						tmpCompsDesc.AppendInNewLine("  - " + conditionalStatAffecter2.statFactors[num3].stat.LabelCap + ": " + conditionalStatAffecter2.statFactors[num3].ToStringAsFactor + " (" + conditionalStatAffecter2.Label + ")");
					}
				}
			}
		}
		...
		{
			for (int num4 = 0; num4 < def.abilityStatFactors.Count; num4++)
			{
				foreach (StatModifier modifier in def.abilityStatFactors[num4].modifiers)
				{
					TryDrawStatLine();
					tmpCompsDesc.AppendInNewLine("  - " + def.abilityStatFactors[num4].ability.LabelCap + ": " + modifier.stat.LabelCap + ": " + modifier.ToStringAsFactor);
				}
			}
		}
		...
		if (ideo.Fluid)
		{
			for (int num5 = 0; num5 < def.comps.Count; num5++)
			{
				if (!(def.comps[num5] is PreceptComp_DevelopmentPoints))
				{
					continue;
				}
				if (flag4)
				...
				foreach (string description3 in def.comps[num5].GetDescriptions())
				{

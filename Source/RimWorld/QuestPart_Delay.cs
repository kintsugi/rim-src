...
public class QuestPart_Delay : QuestPartActivable
{
	...
	public override string AlertLabel
	{
		get
		{
			TaggedString? taggedString = alertLabel?.Formatted(TicksLeft.ToStringTicksToPeriod());
	...
	public override string AlertExplanation
	{
		get
		{
			TaggedString? taggedString = alertExplanation?.Formatted(TicksLeft.ToStringTicksToPeriod());
			if (!taggedString.HasValue)
			{
				return null;
			}

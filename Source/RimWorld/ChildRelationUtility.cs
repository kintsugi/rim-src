...
public static class ChildRelationUtility
{
	...
	public static bool DefinitelyHasNotBirthName(Pawn pawn)
	{
		...
		for (int i = 0; i < spouses.Count; i++)
		{
			...
			if (pawn2.Name is NameTriple nameTriple2)
			{
				string last = nameTriple2.Last;
				if (!(nameTriple.Last != last) && ((pawn2.GetMother() != null && pawn2.GetMother().Name is NameTriple nameTriple3 && nameTriple3.Last == last) || (pawn2.GetFather() != null && pawn2.GetFather().Name is NameTriple nameTriple4 && nameTriple4.Last == last)))
				{
					return true;
				}
			}

...
public class Designator_Dropdown : DesignatorWithEyedropper
{
	...
	private Window SetupGridMenu(Event ev)
	{
		...
		if (elements.Any((Designator x) => x is Designator_Place designator_Place3 && designator_Place3.PlacingDef is TerrainDef terrainDef2 && terrainDef2.designatorDropdown.includeEyeDropperTool))
		{
			if (eyedropper == null)
			{
				eyedropper = new Designator_Eyedropper(delegate(ColorDef newCol)
				{
					for (int j = 0; j < elements.Count; j++)
					{
						...
						if (designator2 is Designator_Place designator_Place2 && designator_Place2.PlacingDef is TerrainDef terrainDef && terrainDef.colorDef == newCol)
						{

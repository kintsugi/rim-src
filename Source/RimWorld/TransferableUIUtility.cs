...
public static class TransferableUIUtility
{
	...
	private static void DoCountAdjustInterfaceInternal(Rect rect, Transferable trad, int index, int min, int max, bool flash, bool readOnly)
	{
		...
		bool flag = trad is TransferableOneWay transferableOneWay && transferableOneWay.HasAnyThing && transferableOneWay.AnyThing is Pawn && transferableOneWay.MaxCount == 1;
	...
	{
		if (!(trad.AnyThing is Pawn pawn) || pawn.guest == null || !pawn.RaceProps.Humanlike)
		{
			return;
		}

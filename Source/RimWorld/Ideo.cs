...
public class Ideo : IExposable, ILoadReferenceable
{
	...
	public List<XenotypeDef> PreferredXenotypes
	{
		get
		{
			if (cachedPreferredXenotypes == null)
			{
				...
				for (int i = 0; i < precepts.Count; i++)
				{
					if (precepts[i] is Precept_Xenotype precept_Xenotype && precept_Xenotype.xenotype != null)
					{
	...
	public List<CustomXenotype> PreferredCustomXenotypes
	{
		get
		{
			if (cachedPreferredCustomXenotypes == null)
			{
				...
				for (int i = 0; i < precepts.Count; i++)
				{
					if (precepts[i] is Precept_Xenotype precept_Xenotype && precept_Xenotype.customXenotype != null)
					{
	...
	public void ExposeData()
	{
		...
		if (hiddenIdeoMode && createdFromNoExpansionGame)
		{
			hiddenIdeoMode = false;
		}
		RecachePrecepts();
	}
	...
	private bool AllRelicsNewlyCollected()
	{
		...
		for (int i = 0; i < precepts.Count; i++)
		{
			if (precepts[i] is Precept_Relic precept_Relic && !precept_Relic.RelicInPlayerPossession)
			{
				return false;
			}
		}
	...
	public Tuple<Precept_Ritual, List<string>> FirstRitualMissingTarget()
	{
		foreach (Precept precept in precepts)
		{
			if (precept is Precept_Ritual precept_Ritual && precept_Ritual.obligationTargetFilter != null)
			{
	...
	public void RegenerateAllApparelRequirements(FactionDef generatingFor = null)
	{
		foreach (Precept item in PreceptsListForReading)
		{
			if (item is Precept_Role precept_Role)
			{

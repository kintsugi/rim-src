...
public class SymbolResolver_WorkSite_Farming : SymbolResolver_WorkSite
{
	...
	public override void Resolve(ResolveParams rp)
	{
		...
		ThingDef thingDef = (rp.stockpileConcreteContents.NullOrEmpty() ? DefDatabase<ThingDef>.AllDefs.Where((ThingDef def) => def.plant != null && def.plant.Sowable).RandomElement() : DefDatabase<ThingDef>.AllDefs.Where((ThingDef def) => def.plant != null && def.plant.harvestedThingDef == rp.stockpileConcreteContents.First().def).First());

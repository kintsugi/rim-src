...
public class WorldPawns : IExposable
{
	...
	private HediffDef DefPreventingMothball(Pawn p)
	{
		...
		{
			if (!hediffs[i].def.AlwaysAllowMothball && !hediffs[i].IsPermanent() && (!(hediffs[i] is Hediff_MissingPart hediff_MissingPart) || hediff_MissingPart.Bleeding) && (!hediffs[i].def.allowMothballIfLowPriorityWorldPawn || !lowPrioritySituations.Contains(situation)))
			{

...
public static class QuestGen_Filter
{
	...
	public static QuestPart_Filter_ThingStudied ThingStudied(this Quest quest, ThingDef thingDef, Action action = null, Action elseAction = null, string inSignal = null, string outSignal = null, string outSignalElse = null, QuestPart.SignalListenMode signalListenMode = QuestPart.SignalListenMode.OngoingOnly)
	{
		QuestPart_Filter_ThingStudied questPart_Filter_ThingStudied = new QuestPart_Filter_ThingStudied();
		questPart_Filter_ThingStudied.thingDef = thingDef;
		questPart_Filter_ThingStudied.inSignal = QuestGenUtility.HardcodedSignalWithQuestID(inSignal) ?? QuestGen.slate.Get<string>("inSignal");
		questPart_Filter_ThingStudied.signalListenMode = signalListenMode;
		questPart_Filter_ThingStudied.outSignal = outSignal;
		questPart_Filter_ThingStudied.outSignalElse = outSignalElse;
		if (action != null)
		{
			QuestGenUtility.RunInner(action, questPart_Filter_ThingStudied.outSignal = QuestGen.GenerateNewSignal("OuterNodeCompleted"));
		}
		if (elseAction != null)
		{
			QuestGenUtility.RunInner(elseAction, questPart_Filter_ThingStudied.outSignalElse = QuestGen.GenerateNewSignal("OuterNodeCompleted"));
		}
		quest.AddPart(questPart_Filter_ThingStudied);
		return questPart_Filter_ThingStudied;
	}

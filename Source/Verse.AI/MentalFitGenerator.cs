...
public class MentalFitGenerator : IExposable
{
	...
	public void Tick()
	{
		...
		else
		{
			...
			{
				if (Rand.MTBEventOccurs(item.CalculateMTBDays(pawn), 60000f, 150f) && pawn.mindState.mentalStateHandler.TryStartMentalState(item.mentalState, null, forceWake: false, causedByMood: false, null, transitionSilently: true))
				{
					break;
				}
			}

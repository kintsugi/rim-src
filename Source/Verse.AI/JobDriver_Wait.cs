...
public class JobDriver_Wait : JobDriver
{
	...
	private void CheckForAutoAttack()
	{
		...
		for (int i = 0; i < 9; i++)
		{
			...
			{
				if (flag && thingList[j] is Pawn pawn && !pawn.Downed && base.pawn.HostileTo(pawn) && !base.pawn.ThreatDisabledBecauseNonAggressiveRoamer(pawn) && GenHostility.IsActiveThreatTo(pawn, base.pawn.Faction))
				{

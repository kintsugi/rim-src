...
public static class ToilFailConditions
{
	...
	{
		f.AddEndCondition(() => (!(f.GetActor().jobs.curJob.GetTarget(ind).Thing is Pawn pawn) || !pawn.InMentalState || pawn.health.hediffSet.HasHediff(HediffDefOf.Scaria)) ? JobCondition.Ongoing : JobCondition.Incompletable);
		return f;
	}

	public static T FailOnAggroMentalState<T>(this T f, TargetIndex ind) where T : IJobEndable
	{
		f.AddEndCondition(() => (!(f.GetActor().jobs.curJob.GetTarget(ind).Thing is Pawn pawn) || !pawn.InAggroMentalState || pawn.health.hediffSet.HasHediff(HediffDefOf.Scaria)) ? JobCondition.Ongoing : JobCondition.Incompletable);
		return f;
	}

	public static T FailOnAggroMentalStateAndHostile<T>(this T f, TargetIndex ind) where T : IJobEndable
	{
		f.AddEndCondition(() => (!(f.GetActor().jobs.curJob.GetTarget(ind).Thing is Pawn pawn) || !pawn.InAggroMentalState || pawn.health.hediffSet.HasHediff(HediffDefOf.Scaria) || !pawn.HostileTo(f.GetActor())) ? JobCondition.Ongoing : JobCondition.Incompletable);
		return f;
	}

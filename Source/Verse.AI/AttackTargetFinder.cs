...
public static class AttackTargetFinder
{
	...
	public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDist = 0f, float maxDist = 9999f, IntVec3 locus = default(IntVec3), float maxTravelRadiusFromLocus = float.MaxValue, bool canBashDoors = false, bool canTakeTargetsCloserThanEffectiveMinRange = true, bool canBashFences = false)
	{
		...
		if (HasRangedAttack(searcher) && (searcherPawn == null || !searcherPawn.InAggroMentalState))
		{
			...
			tmpTargets.RemoveAll((IAttackTarget t) => ShouldIgnoreNoncombatant(searcherThing, t));
		...
		innerValidator = delegate(IAttackTarget t)
		{
			...
			return (!ShouldIgnoreNoncombatant(searcherThing, t)) ? true : false;
		};
	...
	private static bool ShouldIgnoreNoncombatant(Thing searcherThing, IAttackTarget t)
	{
		if (!(t is Pawn pawn))
		{
			return false;
	...
	private static IAttackTarget FindBestReachableMeleeTarget(Predicate<IAttackTarget> validator, Pawn searcherPawn, float maxTargDist, bool canBashDoors, bool canBashFences)
	{
		...
		Func<IntVec3, IAttackTarget> bestTargetOnCell = delegate(IntVec3 x)
		{
			...
			for (int j = 0; j < thingList.Count; j++)
			{
				...
				if (thing is IAttackTarget attackTarget2 && validator(attackTarget2) && ReachabilityImmediate.CanReachImmediate(x, thing, searcherPawn.Map, PathEndMode.Touch, searcherPawn) && (searcherPawn.CanReachImmediate(thing, PathEndMode.Touch) || searcherPawn.Map.attackTargetReservationManager.CanReserve(searcherPawn, attackTarget2)) && !ShouldIgnoreNoncombatant(searcherPawn, attackTarget2))
				{
					return attackTarget2;
				}
			}
	...
	public static void DebugDrawNonCombatantTimer_OnGUI()
	{
		...
		using (new TextBlock(GameFont.Tiny, TextAnchor.MiddleCenter, false))
		{
			...
			{
				if (!(item is Pawn pawn) || pawn.mindState == null)
				{
					continue;
				}

...
public class Pawn_PathFollower : IExposable
{
	...
	private bool PawnCanOccupy(IntVec3 c)
	{
		...
		if (edifice != null && edifice is Building_Door building_Door && !building_Door.PawnCanOpen(pawn) && !building_Door.Open)
		{
			return false;
		}
		return true;
	...
	private void TryEnterNextPathCell()
	{
		...
		if (building != null)
		{
			if (!(building is Building_Door building_Door) || !building_Door.FreePassage)
			{

		...
		toil.initAction = delegate
		{
			...
			else
			{
				...
				if (thingOwner != null)
				{
					...
					if (num3 != 0)
					{
						if (thing is Building_Grave building_Grave)
						{
							building_Grave.Notify_CorpseBuried(actor);
						}
						else if (thing is Building_GibbetCage building_GibbetCage)
						{
							building_GibbetCage.Notify_CorpseAdded();
						}
						if (ModsConfig.IdeologyActive && thing is ThingWithComps thing3 && thing3.TryGetComp<CompBiosculpterPod>() != null)
						{
							thing3.TryGetComp<CompBiosculpterPod>().Notify_NutritionAdded();
						}
						if (ModsConfig.BiotechActive)
						{
							if (thing is Building_MechGestator building_MechGestator)
							{

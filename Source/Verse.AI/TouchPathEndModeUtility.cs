...
public static class TouchPathEndModeUtility
{
	...
	{
		if (cell == BL && !IsCornerTouchAllowed(BL.x + 1, BL.z + 1, BL.x + 1, BL.z, BL.x, BL.z + 1, pc))
		{
			return true;
		}
		if (cell == TL && !IsCornerTouchAllowed(TL.x + 1, TL.z - 1, TL.x + 1, TL.z, TL.x, TL.z - 1, pc))
		{
			return true;
		}
		if (cell == TR && !IsCornerTouchAllowed(TR.x - 1, TR.z - 1, TR.x - 1, TR.z, TR.x, TR.z - 1, pc))
		{
			return true;
		}
		if (cell == BR && !IsCornerTouchAllowed(BR.x - 1, BR.z + 1, BR.x - 1, BR.z, BR.x, BR.z + 1, pc))
		{
			return true;
		}
		return false;
	...
	public static void AddAllowedAdjacentRegions(LocalTargetInfo dest, TraverseParms traverseParams, Map map, List<Region> regions)
	{
		...
		if (!dest.HasThing || (dest.Thing.def.size.x == 1 && dest.Thing.def.size.z == 1))
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				if (intVec.InBounds(map) && !IsAdjacentCornerAndNotAllowed(intVec, BL, TL, TR, BR, pc))
				{
		...
		{
			if (list[j].InBounds(map) && !IsAdjacentCornerAndNotAllowed(list[j], BL, TL, TR, BR, pc))
			{
	...
	public static bool IsAdjacentOrInsideAndAllowedToTouch(IntVec3 root, LocalTargetInfo target, PathingContext pc)
	{
		...
		{
			return !IsAdjacentCornerAndNotAllowed(root, BL, TL, TR, BR, pc);
		}
		return false;
	}
}

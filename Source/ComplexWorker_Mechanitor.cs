	...
	private bool TryPlaceDeceasedMechanitor(CellRect room, Map map, out Building_AncientCryptosleepPod casket)
	{
		foreach (IntVec3 item in room.Cells.InRandomOrder())
		{
			if (CanPlaceCasketAt(item))
			{
				...
				obj2.letter = LetterMaker.MakeLetter("LetterLabelMechanitorCasketOpened".Translate(pawn), "LetterMechanitorCasketOpened".Translate(pawn), LetterDefOf.NeutralEvent, pawn.Corpse);
				...
				obj4.letter = LetterMaker.MakeLetter("LetterLabelMechanitorCasketFound".Translate(), "LetterMechanitorCasketFound".Translate(), LetterDefOf.NeutralEvent, casket);

...
public sealed class AutoSlaughterManager : IExposable
{
			...
			if (cacheDirty)
			{
				try
				{
					...
					foreach (AutoSlaughterConfig config in configs)
					{
						...
						tmpAnimals.SortBy((Pawn a) => a.ageTracker.AgeBiologicalTicks);
						tmpAnimalsMale.SortBy((Pawn a) => a.ageTracker.AgeBiologicalTicks);
						tmpAnimalsMaleYoung.SortBy((Pawn a) => a.ageTracker.AgeBiologicalTicks);
						tmpAnimalsFemale.SortBy((Pawn a) => a.ageTracker.AgeBiologicalTicks);
						tmpAnimalsFemaleYoung.SortBy((Pawn a) => a.ageTracker.AgeBiologicalTicks);
						if (config.allowSlaughterPregnant)
						{
							tmpAnimalsPregnant.SortBy((Pawn a) => 0f - a.health.hediffSet.GetFirstHediffOfDef(HediffDefOf.Pregnant).Severity);
						...
						if (config.maxFemales != -1)
						{
							...
							{
								Pawn item = tmpAnimalsFemale.Pop();
						...
						if (config.maxFemalesYoung != -1)
						{
							...
							{
								Pawn item2 = tmpAnimalsFemaleYoung.Pop();
						...
						if (config.maxMalesYoung != -1)
						{
							...
							{
								Pawn item4 = tmpAnimalsMaleYoung.Pop();
	...
	public void ExposeData()
	{
		...
		{
			if (configs.RemoveAll((AutoSlaughterConfig x) => x.animal == null) != 0)
			{
	...
	private void TryPopulateMissingAnimals()
	{
		...
		{
			if (allDef.race != null && allDef.race.Animal && allDef.race.wildness < 1f && !allDef.race.Dryad && !hashSet.Contains(allDef))
			{

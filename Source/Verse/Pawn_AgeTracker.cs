...
public class Pawn_AgeTracker : IExposable
{
	...
	private void BirthdayBiological(int birthdayAge)
	{
		...
		if (pawn.RaceProps.Humanlike)
		{
			...
			if (!PawnUtility.ShouldSendNotificationAbout(pawn) || pawn.Faction != Faction.OfPlayer || pawn.IsQuestLodger())
			{
				...
				{
					SkillDef skillDef = ChoiceLetter_GrowthMoment.PassionOptions_NewTemp(pawn, passionGainsCount, checkGenes: true).FirstOrFallback();
					if (skillDef != null)
					{
			...
			else if (tmpHediffsGained.Count > 0 || flag3)
			{
				...
				{
					Name name = pawn.Name;
					if (pawn.Name is NameTriple nameTriple && !nameTriple.NickSet)
					{
	...
	{
		if (!(pawn.Name is NameSingle nameSingle) || !nameSingle.Numerical)
		{
			return;
		}

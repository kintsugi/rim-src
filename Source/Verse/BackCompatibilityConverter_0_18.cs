...
public class BackCompatibilityConverter_0_18 : BackCompatibilityConverter
{
	...
	public override void PostExposeData(object obj)
	{
		if (Scribe.mode == LoadSaveMode.PostLoadInit)
		{
			...
			if (obj is Pawn pawn && !pawn.Destroyed && !pawn.Dead && pawn.needs == null)
			{
				Log.Error(pawn.ToStringSafe() + " has null needs tracker even though he's not dead. Fixing...");
				pawn.needs = new Pawn_NeedsTracker(pawn);
				pawn.needs.SetInitialLevels();
			}
			if (obj is History history && history.archive == null)
			{
				history.archive = new Archive();
			}
			if (obj is WorldInfo worldInfo && worldInfo.persistentRandomValue == 0)
			{
		...
		if (Scribe.mode == LoadSaveMode.LoadingVars)
		{
			...
			if (obj is WorldFeature worldFeature && worldFeature.maxDrawSizeInTiles == 0f)
			{
		...
		if (obj is Hediff hediff2 && hediff2.temp_partIndexToSetLater >= 0 && hediff2.pawn != null)
		{

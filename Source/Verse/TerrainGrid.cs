...
public sealed class TerrainGrid : IExposable
{
	...
	private void ExposeColorGrid()
	{
		...
		Func<IntVec3, ushort> shortReader = (IntVec3 c) => colorGrid[map.cellIndices.CellToIndex(c)]?.shortHash ?? 0;
		Action<IntVec3, ushort> shortWriter = delegate(IntVec3 c, ushort val)
		{
		...
		MapExposeUtility.ExposeUshort(map, shortReader, shortWriter, "colorGrid");
	}

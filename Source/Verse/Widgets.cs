...
public static class Widgets
{
	...
	public static void DefIcon(Rect rect, Def def, ThingDef stuffDef = null, float scale = 1f, ThingStyleDef thingStyleDef = null, bool drawPlaceholder = false, Color? color = null, Material material = null, int? graphicIndexOverride = null)
	{
		...
		if (def is ThingDef thingDef && thingDef.IsFrame && thingDef.entityDefToBuild != null)
		{
		...
		else if (def is RecipeDef recipeDef && recipeDef.UIIconThing != null)
		{
	...
	public static void HorizontalSlider(Rect rect, ref float value, FloatRange range, string label = null, float roundTo = -1f)
	{
		...
		value = HorizontalSlider_NewTemp(rect, value, trueMin, trueMax, middleAlignment: false, label, trueMin.ToString(), trueMax.ToString(), roundTo);
	}
	...
	{
		return HorizontalSlider_NewTemp(rect, value, leftValue, rightValue, middleAlignment, label, leftAlignedLabel, rightAlignedLabel, roundTo);
	}
	...
	public static float FrequencyHorizontalSlider(Rect rect, float freq, float minFreq, float maxFreq, bool roundToInt = false)
	{
		...
		float num2 = HorizontalSlider_NewTemp(rect, num, 0f, 1f, middleAlignment: true, label);
		if (num != num2)
		{
			float num3;
			if (num2 < 0.5f)

...
public abstract class Window
{
	...
	private void InnerWindowOnGUI(int x)
	{
		...
		QuickSearchWidget commonSearchWidget = CommonSearchWidget;
		if (commonSearchWidget != null)
		{
			Rect rect4 = new Rect(rect.x + commonSearchWidgetOffset.x, rect.height - 55f + commonSearchWidgetOffset.y, QuickSearchSize.x, QuickSearchSize.y);
			commonSearchWidget.OnGUI(rect4, notify_CommonSearchChangedCached);
		}
		...
		if (doCloseButton)
		{
			...
			if (Widgets.ButtonText(new Rect(rect.width / 2f - CloseButSize.x / 2f, rect.height - 55f, CloseButSize.x, CloseButSize.y), CloseButtonText))
			{
				Close();
			}
		}

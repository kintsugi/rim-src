...
public class PollutionGrid : IExposable
{
	...
	public void SetPolluted(IntVec3 cell, bool isPolluted, bool silent = false)
	{
		if (ModLister.CheckBiotech("Set pollution") && cell.InBounds(map) && grid[cell] != isPolluted)
		{
			...
			dirty = true;
			map.mapDrawer.MapMeshDirty(cell, MapMeshFlag.Terrain, regenAdjacentCells: true, regenAdjacentSections: false);
			map.mapDrawer.MapMeshDirty(cell, MapMeshFlag.Pollution, regenAdjacentCells: false, regenAdjacentSections: false);
			Drawer.SetDirty();

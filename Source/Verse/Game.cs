...
public class Game : IExposable
{
	...
	{
		DeinitAndRemoveMap_NewTemp(map, notifyPlayer: true);
	}

	public void DeinitAndRemoveMap_NewTemp(Map map, bool notifyPlayer)
	{
		if (map == null)
		{
		...
		MapDeiniter.Deinit_NewTemp(map, notifyPlayer);
		maps.Remove(map);
		if (currentMap != null)
		{

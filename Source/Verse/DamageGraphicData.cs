	...
	public void ResolveReferencesSpecial()
	{
		LongEventHandler.ExecuteWhenFinished(delegate
		{
			if (scratches != null)
			{
				...
				{
					scratchMats[i] = MaterialPool.MatFrom(scratches[i], ShaderDatabase.Transparent, 2905);
					GlobalTextureAtlasManager.TryInsertStatic(TextureAtlasGroup.Building, (Texture2D)scratchMats[i].mainTexture);
				}
			}
			if (cornerTL != null)
			{

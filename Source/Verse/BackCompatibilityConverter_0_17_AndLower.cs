...
public class BackCompatibilityConverter_0_17_AndLower : BackCompatibilityConverter
{
	...
	public override void PostExposeData(object obj)
	{
		...
		{
			if (obj is Game game && game.battleLog == null)
			{
		...
		{
			if (obj is Map map && map.pawnDestinationReservationManager == null)
			{
				map.pawnDestinationReservationManager = new PawnDestinationReservationManager();
			}
			if (obj is Pawn pawn && pawn.Spawned && pawn.rotationTracker == null)
			{
				pawn.rotationTracker = new Pawn_RotationTracker(pawn);
			}
			if (obj is Pawn_MindState pawn_MindState && pawn_MindState.inspirationHandler == null)
			{
				pawn_MindState.inspirationHandler = new InspirationHandler(pawn_MindState.pawn);
			}
			if (obj is ImportantPawnComp importantPawnComp && importantPawnComp.pawn == null)
			{
				importantPawnComp.pawn = new ThingOwner<Pawn>(importantPawnComp, oneStackOnly: true);
			}
			if (obj is WorldPawns worldPawns && worldPawns.gc == null)
			{
				worldPawns.gc = new WorldPawnGC();
			}
			if (obj is GameCondition gameCondition && !gameCondition.Permanent && gameCondition.Duration > 1000000000)
			{
				gameCondition.Permanent = true;
			}
			if (obj is Building_TurretGun building_TurretGun && building_TurretGun.gun == null)
			{

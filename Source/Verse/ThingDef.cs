...
public class ThingDef : BuildableDef
{
	...
	public bool EverTransmitsPower
	{
		get
		{
			...
			{
				if (comps[i] is CompProperties_Power compProperties_Power && compProperties_Power.transmitsPower)
				{
					return true;
				}
			}
	...
	public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req)
	{
		...
		if (IsMeat)
		{
			...
			string valueString2 = string.Join(", ", list.Select((ThingDef p) => p.label).ToArray()).CapitalizeFirst();
			yield return new StatDrawEntry(StatCategoryDefOf.BasicsPawn, "Stat_SourceSpecies_Name".Translate(), valueString2, "Stat_SourceSpecies_Desc".Translate(), 1200, null, Dialog_InfoCard.DefsToHyperlinks(list));
		}
		if (IsLeather)
		{
			...
			string valueString3 = string.Join(", ", list2.Select((ThingDef p) => p.label).ToArray()).CapitalizeFirst();
			yield return new StatDrawEntry(StatCategoryDefOf.BasicsPawn, "Stat_SourceSpecies_Name".Translate(), valueString3, "Stat_SourceSpecies_Desc".Translate(), 1200, null, Dialog_InfoCard.DefsToHyperlinks(list2));
		}
		if (!equippedStatOffsets.NullOrEmpty())
		{
			for (int k = 0; k < equippedStatOffsets.Count; k++)
			{
				...
				float num2 = equippedStatOffsets[k].value;
				...
				yield return new StatDrawEntry(StatCategoryDefOf.EquippedStatOffsets, equippedStatOffsets[k].stat, num2, StatRequest.ForEmpty(), ToStringNumberSense.Offset, null, forceUnfinalizedMode: true).SetReportText(stringBuilder4.ToString());
			}
		}
		if (!IsDrug)
		{

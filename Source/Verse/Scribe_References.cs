...
public static class Scribe_References
{
	public static void Look<T>(ref T refee, string label, bool saveDestroyedThings = false) where T : ILoadReferenceable
	{
		if (Scribe.mode == LoadSaveMode.Saving)
		{
			...
			else if ((!((object)refee is Thing th) || !CheckSaveReferenceToDestroyedThing(th, label, saveDestroyedThings)) && (!((object)refee is WorldObject w) || !CheckSaveReferenceToDestroyedWorldObject(w, label, saveDestroyedThings)))
			{

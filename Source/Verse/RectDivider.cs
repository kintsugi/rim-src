...
public struct RectDivider
{
	...
	public RectDivider NewRow(float height, VerticalJustification justification = VerticalJustification.Top)
	{
		...
		{
			if (!currentRect.SplitHorizontallyWithMargin(out var top2, out var bottom2, out var overflow2, margin.y, height))
			{
		...
		{
			if (!currentRect.SplitHorizontallyWithMargin(out var top, out var bottom, out var overflow, margin.y, null, height))
			{
	...
	public RectDivider NewCol(float width, HorizontalJustification justification = HorizontalJustification.Left)
	{
		...
		{
			if (!currentRect.SplitVerticallyWithMargin(out var left2, out var right2, out var overflow2, margin.x, width))
			{
		...
		{
			if (!currentRect.SplitVerticallyWithMargin(out var left, out var right, out var overflow, margin.x, null, width))
			{

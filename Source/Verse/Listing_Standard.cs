...
public class Listing_Standard : Listing
{
	...
	{
		float result = Widgets.HorizontalSlider_NewTemp(GetRect(22f), val, min, max);
		Gap(verticalSpacing);
		return result;
	}
	...
	public float SliderLabeled(string label, float val, float min, float max, float labelPct = 0.5f, string tooltip = null)
	{
		...
		Text.Anchor = TextAnchor.UpperLeft;
		float result = Widgets.HorizontalSlider_NewTemp(rect.RightPart(1f - labelPct), val, min, max, middleAlignment: true);
		Gap(verticalSpacing);
		return result;
	}

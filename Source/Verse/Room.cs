...
public class Room
{
	...
	public void Notify_RoomShapeChanged()
	{
		...
		if (Building_Bed.RoomCanBePrisonCell(this))
		{
			...
			{
				if (containedAndAdjacentThings[i] is Building_Bed building_Bed && building_Bed.ForPrisoners)
				{
					isPrisonCell = true;
					break;
				}
	...
	public string GetRoomRoleLabel()
	{
		...
		TaggedString taggedString = ((pawn == null) ? ((TaggedString)Role.PostProcessedLabelCap) : ((pawn2 != null && pawn2 != pawn) ? "CouplesRoom".Translate(pawn.LabelShort, pawn2.LabelShort, Role.label, pawn.Named("PAWN1"), pawn2.Named("PAWN2")) : "SomeonesRoom".Translate(pawn.LabelShort, Role.label, pawn.Named("PAWN"))));
		return taggedString;
	}

...
public static class MapDeiniter
{
	...
	{
		Deinit_NewTemp(map, notifyPlayer: true);
	}

	public static void Deinit_NewTemp(Map map, bool notifyPlayer)
	{
		try
		{
		...
		try
		{
			PassPawnsToWorld_NewTemp(map, notifyPlayer);
		}
		catch (Exception ex2)
		{
	...
	{
		PassPawnsToWorld_NewTemp(map, notifyPlayer: true);
	}

	private static void PassPawnsToWorld_NewTemp(Map map, bool notifyPlayer)
	{

...
public static class DebugToolsPawns
{
			...
			select d)
		{
			...
			list.Add(new DebugActionNode(localDef.defName)
			{
				action = delegate
				{
					...
					{
						if (item3 is Pawn pawn && pawn.apparel != null)
						{
			...
			select d)
		{
			...
			list.Add(new DebugActionNode(thingDef.defName)
			{
				action = delegate
				{
					...
					{
						if (item2 is Pawn pawn && pawn.equipment != null)
						{
	...
	[DebugAction("Pawns", "Create baby from parents", false, false, false, 0, false, actionType = DebugActionType.Action, allowedGameStates = AllowedGameStates.PlayingOnMap, requiresBiotech = true)]

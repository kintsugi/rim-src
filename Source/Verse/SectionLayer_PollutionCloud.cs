...
public class SectionLayer_PollutionCloud : SectionLayer_Gas
{
	...
	public override Material Mat
	{
		get
		{
			if (matCached == null)
			{
				...
				matCached.SetTexture("_FadeTex", PollutionFadeTex.Texture);
				matCached.SetVector("_FadeTexScrollSpeed", FadeTexScrollSpeed);
				matCached.SetVector("_FadeTexScale", FadeTexScale);
			}
			return matCached;
		}
	}
	...
	public SectionLayer_PollutionCloud(Section section)
		: base(section)
	{
		relevantChangeTypes = MapMeshFlag.Pollution;
	}

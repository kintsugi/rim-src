...
public static class GrammarResolver
{
	...
	private static List<RuleEntry> tmpSortedRuleList = new List<RuleEntry>();
	...
	private static RuleEntry RandomPossiblyResolvableEntry(string keyword, Dictionary<string, string> constants, List<string> extraTags, List<string> resolvedTags, GrammarRequest.ICustomizer customizer)
	{
		...
		IComparer<Rule> customComparer = customizer?.StrictRulePrioritizer();
		...
		return list.RandomElementByWeightWithFallback((RuleEntry rule) => (!ValidateRule(constants, extraTags, resolvedTags, rule, customizer) || rule.Priority != maxPriority) ? 0f : rule.SelectionWeight);
	}
